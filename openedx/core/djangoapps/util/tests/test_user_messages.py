...
from ..user_messages import UserMessageCollection, UserMessageType
...
class UserMessagesTestCase(TestCase):
    ...
    def test_message_escaping(self, message, expected_message_html):
        ...
        """
        UserMessageCollection.register_user_message(self.request, UserMessageType.INFO, message)
        messages = list(UserMessageCollection.user_messages(self.request))
    ...
    def test_message_icon(self, message_type, expected_css_class, expected_icon_class):
        ...
        """
        UserMessageCollection.register_user_message(self.request, message_type, TEST_MESSAGE)
        messages = list(UserMessageCollection.user_messages(self.request))
        self.assertEqual(len(messages), 1)
    ...
    @ddt.data(
        (UserMessageCollection.register_error_message, UserMessageType.ERROR),
        (UserMessageCollection.register_info_message, UserMessageType.INFO),
        (UserMessageCollection.register_success_message, UserMessageType.SUCCESS),
        (UserMessageCollection.register_warning_message, UserMessageType.WARNING),
    )
    @ddt.unpack
